...
export async function findTestFiles(
    patterns: string | string[] = ['**/*.test.ts', '**/*.test.tsx', '**/*.spec.ts', '**/*.spec.tsx'],
    options: { exclude?: string[]; cwd?: string } = {}
): Promise<string[]> {
    const { exclude = ['**/node_modules/**', '**/dist/**', '**/build/**'], cwd = process.cwd() } = options;
 ...
 */
export function findPreloadPath(cwd: string = process.cwd()): string | undefined {
    const candidates = [
...
export async function runTestFile(
    ...
    const testPath = relativePath.startsWith('.') ? relativePath : `.${path.sep}${relativePath}`;
...
export async function runIsolated(
    ...
    console.log(`   Workers: ${parallel} | Platform: ${process.platform} | Preload: ${preloadPath ? 'yes' : 'no'}\n`);
