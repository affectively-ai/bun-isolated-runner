...
async function main(): Promise<void> {
    ...
    let timeout = parseInt(process.env['BUN_ISOLATED_TIMEOUT'] || '', 10) || 30000;
    let retries = 0;
    let verbose = false;
    let cwd = process.cwd();
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--help' || arg === '-h') {
            ...
            cwd = arg.split('=')[1];
        } else if (arg.endsWith('.test.ts') || arg.endsWith('.test.tsx') || arg.endsWith('.spec.ts') || arg.endsWith('.spec.tsx')) {
